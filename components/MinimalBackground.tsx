...
function Particles() {
  ...
  useEffect(() => {
    ...
    grad.addColorStop(0, "rgba(0,83,206,0.9)");
    grad.addColorStop(0.4, "rgba(0,83,206,0.5)");
    grad.addColorStop(1, "rgba(0,83,206,0)");
    sctx.fillStyle = grad;
    ...
    const animate = () => {
      ...
      for (let i = 0; i < COUNT; i++) {
        ...
        if (y > 8.5) y = -8.5; // wrap around for endless float
        pos.setY(i, y);
        pos.setX(i, pos.getX(i) + Math.sin(t + sway[i]) * 0.0025);
      }
      pos.needsUpdate = true;
      points.rotation.y = Math.sin(t * 0.08) * 0.12;
